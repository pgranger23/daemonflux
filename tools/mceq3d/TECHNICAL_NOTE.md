...
**Performance.** On a single core, one MCEq cascade solve is ≈1.5 s; a full directional solve over a 48-direction (6×8) sky grid is ≈6 min — about 240× one MCEq solve, or ≈14× the equivalent 1D MCEq flux. The cost is ~85 % geomagnetic-cutoff back-tracing; the daemonflux base itself is ~10 ms. Both heavy terms are reusable — the cutoff map is a one-time per-site precompute (cached; ~0.3 µs/direction to re-evaluate) and G_s is site/zenith-independent — so after a one-time precompute, evaluation is near-instant, versus the CPU-weeks of a full 3D Monte-Carlo.
