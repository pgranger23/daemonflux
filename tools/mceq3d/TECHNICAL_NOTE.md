...
The directional observables, which are ratios and therefore independent of the absolute base, are reproduced well. The engine recovers the **up/down asymmetry** (up-going flux exceeds down-going at fixed energy, as in Honda), the **sec θ horizon enhancement**, which now tracks Honda to ≈3–6 % across 0.3–100 GeV (1.27 vs 1.23 at 1 GeV; earlier drafts showed a prototype curve without E_off that undershot below a few GeV). The **East–West amplitude** has the correct sign and >10 GeV vanishing but the delivered engine **undershoots** Honda's near-horizon max/min-azimuth amplitude by ≈20 % at 0.5–1 GeV (2.02 vs 2.51 at 0.5 GeV, 1.69 vs 2.09 at 1 GeV), traced NOT to the penumbra width (tested: sharpening it saturates ~18% below Honda) but to a genuinely-3D coupling the factorisation omits -- the cutoff is applied at the neutrino direction while the parent primary arrives from the production cone, each direction with its own cutoff; the principled fix is to average G over that cone (the geomagnetic analogue of E_off), a scoped extension, not yet implemented (§6). The **νe/νμ flavour ratio** — the sharpest, most model-independent test, set by the π→μ→e chain — agrees with Honda to 0.7–4 % across the band.
