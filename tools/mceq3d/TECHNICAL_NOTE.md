...
`daemonflux` provides a data-driven, muon-calibrated parametrisation of the *one-dimensional* inclusive atmospheric lepton flux, with a rigorous covariance-based error model. Its 1D assumption (neutrinos collinear with the primary) is excellent at high energy but breaks down below a few GeV, precisely the region that dominates atmospheric-neutrino oscillation measurements, where (i) the geomagnetic rigidity cutoff shapes the flux strongly and anisotropically, (ii) the Earth's curvature and the finite production-angle of secondaries make the problem genuinely three-dimensional, and (iii) muons bend in the geomagnetic field before decaying. This note describes an extension that produces an **absolute, all-flavour, directional flux** Φ(E, cos θ_z, φ) over the full sky from **0.1 to 100 GeV**, built so that each ingredient is either taken from a trusted source or validated against data. The core construction is the complete deterministic-3D product Φ = Φ₁D(curved per-zenith) × E_off(E,cosθ) × G(E,R_c(θ,φ)) × S(E): a per-zenith curved-atmosphere cascade, a **first-principles off-axis 3D-production factor** E_off, a cascade-correct geomagnetic suppression with a first-principles IGRF back-traced cutoff, and an optional solar factor. E_off is the complete genuine-3D/1D production ratio — built from MCEq depth-resolved production, curved-atmosphere geometry, and the **pion production angle taken directly from the SIBYLL/UrQMD generator moments** (via chromo, NA61-validated) folded with exact π→μν decay and the H3a spectrum (no hand-set p_T/x_F) — and it produces the large sub-GeV near-horizon excess (horizon/vertical ≈1.8 at 0.3 GeV). The excess is a production-rate effect inherited by all daughters, so E_off is flavour-independent, reproducing the Honda and Bartol νμ and νe zenith shapes to within their mutual ≈5–15 %. Against the Honda HKKM2014 Kamioka tables the engine reproduces the absolute νμ normalisation to ≈10 % above 1 GeV (with a data-anchored base), the sec θ horizon enhancement to ≈3–6 % (1.27 vs Honda's 1.23 at 1 GeV) and the νe/νμ flavour ratio to better than 4 %; the East–West asymmetry, using a production-cone-averaged geomagnetic cutoff and compared at a *matched* near-horizon zenith, is reproduced to ≈6–14 % at moderate zenith (validated out-of-sample across zenith bands), with a systematic overshoot growing toward the extreme horizon where the production cone truncates at the limb — a large improvement over the ≈20 % deficit of earlier drafts, whose dominant cause was a rigidity-scan cap that clamped the near-horizon East cutoff, not the geomagnetic penumbra (§4.1). The dominant residual uncertainty is the genuine sub-GeV spread between hadronic/flux models, which we quantify and deliver as an explicit, energy-dependent systematic band. We are explicit throughout about what is production-trustable and what remains a research-grade approximation.
...
The directional observables, which are ratios and therefore independent of the absolute base, are reproduced well. The engine recovers the **up/down asymmetry** (up-going flux exceeds down-going at fixed energy, as in Honda), the **sec θ horizon enhancement**, which now tracks Honda to ≈3–6 % across 0.3–100 GeV (1.27 vs 1.23 at 1 GeV; earlier drafts showed a prototype curve without E_off that undershot below a few GeV). The **East–West amplitude** has the correct sign, peak and >10 GeV vanishing, and — compared at a *matched* near-horizon zenith (cosZ 0.25, ~75°, not the earlier mismatched 75°-vs-horizon-bin comparison) — sits ≈11–14 % above Honda across 0.4–2 GeV (2.53 vs 2.23 at 0.6 GeV, 2.10 vs 1.85 at 1.1 GeV), a systematic overshoot at the edge of the Honda–Bartol spread, and a large improvement over the pre-fix ≈14 % deficit at this zenith. An **out-of-sample** zenith scan (bands not used in the diagnosis) gives ≈5–6 % at 63°, ≈8–11 % at 69°, growing to ≈25–29 % at the extreme horizon (87°), where the ~20° production cone extends below the limb and the down-going cutoff map is clamped (an understood, localised limitation). The deficit's dominant cause was pinned down: (i) a **rigidity-scan cap** — the back-trace scanned only to 20 GV, but the near-horizon East cutoff at Kamioka exceeds that and was clamped (the penumbra width was tested and ruled out); raising the ceiling to 40 GV restores the contrast; and (ii) **production-cone averaging** — averaging G over the production cone (the geomagnetic analogue of E_off) smears the maximally-sharp single cutoff toward Honda. Both are implemented and default-on (`cone_cutoff=True`, on a cached full-sky IGRF cutoff map). The **νe/νμ flavour ratio** — the sharpest, most model-independent test, set by the π→μ→e chain — agrees with Honda to 0.7–4 % across the band.
...
The dominant uncertainty below a few GeV is not numerical but physical: the *interaction-model* ambiguity is small (≈4–7 % sub-GeV; SIBYLL/EPOS/DPMJET/QGSJET) and the full-3D references agree well (Honda–Bartol ≈4–15 %), but the *overall-normalisation/primary/calibration* ambiguity is larger. Rather than hide it, we quantify the latter from the spread of the two bases and deliver it as an energy-dependent one-sigma band: ±37 % at 0.1 GeV, ±31 % at 0.5 GeV, ±21 % at 1 GeV, ±10 % at 10 GeV, ±4 % at 100 GeV, to be carried through an oscillation fit as a correlated normalisation-vs-energy systematic. Two caveats: the bases bracket Honda only below ~1 GeV (above, both lie below it, daemonflux being the closer central), and this two-model spread is an intra-framework floor on the flux uncertainty — a complete systematic should also fold in the inter-calculation (Honda/Bartol/FLUKA) envelope, which at multi-GeV lies ~10–18 % above our geometric-mean central. Decomposing the spread (`hadronic_spread.py`, running SIBYLL-2.3d, EPOS-LHC, DPMJET-III, QGSJET-II through the same setup) shows the pure **interaction-model** contribution is only ~4–7 % sub-GeV (rising to ~12–13 % at multi-GeV) — much smaller than the calibration-driven spread — so the sub-GeV uncertainty is dominated by overall normalisation, not by the interaction model (the literal Bartol comparison above corroborates this — Honda and Bartol agree to ~4%). A distinct, time-dependent effect, the 11-year **solar modulation** of the sub-GeV primary, is included as an optional force-field knob (`solve(solar_modulation=φ)`): the predicted solar min/max amplitude is ~+22 % at 0.3 GeV, +14 % at 1 GeV, +2 % at 10 GeV, matching Honda/Bartol. Other sub-dominant, well-bounded effects include the production-angle treatment (~1–2 %), the zenith-independence of G_s (≤2 %), the per-nucleus rigidity approximation (the bound ⟨A/Z⟩ is composition-averaged), and the muon-bending kinematic constants (E_μ ≈ 3E_ν, the muon charge ratio), which matter only for fine-grained charge-resolved studies.

---

## 5. Scope and limitations

We are deliberately explicit about the trust boundary. The **delivered engine** is the complete deterministic-3D product (curved per-zenith cascade × first-principles off-axis production E_off × geomagnetic cutoff × solar, `solve(offaxis=True)`), validated absolutely against Honda and Bartol and at the kinematic level against NA61. The large *net* off-axis production geometry — the **sub-GeV near-horizon excess** (horizon/vertical ≈1.8 at 0.3 GeV; Honda and Bartol agree on it to ≈5–15%) — is now produced **from first principles** by E_off (`offaxis_mc.py`, `kinematic_kernel.py`): MCEq depth-resolved production p(X,E), curved-atmosphere slant depth, and the **pion production angle from the SIBYLL/UrQMD generator moments** (chromo, pooled by `fokker_planck_3d.load_theta2`, NA61-validated) folded with exact π→μν decay and the H3a spectrum — **no hand-set p_T or x_F**. The excess is a production-rate effect set by where the parent pions are produced, so it is inherited by every daughter neutrino and E_off is **flavour-independent** (one factor on each flavour's own 1D base; the near-isotropic muon-decay νμ carry the same excess — they are *not* a flat pedestal). **No reference flux enters**; the derived zenith shape reproduces Honda and Bartol to within their mutual ~5–15% across 0.3–10 GeV for **both νμ and νe** (0.3 GeV horizon, base×E_off vs Honda[Bartol]: νμ 1.82 vs 1.89[1.76], νe 2.06 vs 2.18[2.01]), 1D limit recovered exactly at high energy. The much larger νe flux horizon-enhancement is entirely in the more horizon-peaked (muon-decay-origin) νe base. This supersedes the earlier reference-anchored correction. Its uncertainty is data-driven — NA61 ±12% on the pion angle → ±8% on the sub-GeV horizon excess (→0 by a few GeV), delivered as the `sigma_pi_NA61` nuisance pull; the multi-generator (EPOS/QGSJET) spread needs regenerating the moments. **Muon-calibration consistency**: the **full** E_off is applied on every base (including daemonflux). A built-in closure check justifies this: E_off evaluated with the **muon** kernel (massive-daughter kinematics) is 0.998–1.000 at the vertical for E_μ ≥ 5 GeV (1.02→1.00 at the horizon over 5–30 GeV), i.e. it leaves daemonflux's muon-calibration region untouched. daemonflux's neutrino flux is itself genuinely 1D, so with E_off_μ≈1 there the calibrated yields hold no hidden 3D normalisation for E_off_ν to double-count; the full factor is self-consistent and tracks Honda better than dividing out the vertical (sub-GeV vertical ≈1.0× vs ≈1.07× Honda). The deeper fix — injecting the calibration at the MCEq yields so muons and neutrinos share one cascade — is noted as future work; daemonflux ships flux-gradient splines rather than the Barr-yield recipe, so it needs its build code. The delivered covariance now carries **all** systematics as named pulls appended to the 24 calibration parameters: `sigma_pi_NA61`, `solar_phi` (chosen 1σ potential), and `base_model_spread` (half log-spread MCEq↔daemonflux, the dominant sub-GeV term) — block-diagonal correlation, folded into `flux_relerr`/`flux_err` in quadrature. The cone uses the sampled angular distribution from the generator's full (x_L, θ) pion kernel + exact decay (no Gaussian assumption; fallback only where statistics are thin), and the kaon parent enters as a separate term (own MCEq depth profile, σ_K); a full continuous off-axis transport remains the ultimate refinement. The E_off table is verified by unit tests (`test_offaxis_mc.py`, `test_kinematic_kernel.py`: collimated limit →1, horizon excess sign, NA61 bracketing, muon closure, hadronic-tag integrity) and by `verify_offaxis.py`: the p(X,E) zenith-independence holds to ≤0.2% (60°) / 2.4% (85°), the rigidity-cutoff (site) independence to 1.1% at 11.3 GV, and the cone quadrature is converged to 0.11% — all well inside the ±8% kernel systematic. Known approximations, all bounded above: the up-going hemisphere uses a single representative far-side production point per direction; the high-statistics SIBYLL kernels degrade near the generator's soft-pion threshold (irrelevant to the angular moments but not usable for standalone absolute yields); meson re-interactions and a few muon-bending kinematic constants are fixed rather than derived. daemonflux's muon-calibration **covariance** is now propagated to the directional flux (`solve(with_calib_error=True)`): since G and S are parameter-independent the fractional error carries through exactly (verified <0.1%), giving a ~3–5% calibration band on νμ at Kamioka, complementary to the (larger, sub-GeV) model-spread band. `with_calib_jacobian=True` additionally exports the 24 nuisance-parameter names, their correlation matrix, and the per-parameter Jacobian (`calib_covariance` builds the full energy–energy covariance), reproducing daemonflux's `error()` to machine precision — so a fit can carry the correlated pulls directly on the 3D flux. One capability remains out of scope: the **correlated self-veto muon** accompanying a down-going neutrino is an event-level quantity an inclusive flux model cannot provide (it needs a dedicated tool such as nuVeto). The same base also exposes the inclusive **muon** flux, so a directional muon flux is a trivial add-on.
...
This extension bridges the gap between a calibrated 1D spline model and a full 3D Monte-Carlo: it produces an absolute, all-flavour, directional atmospheric-neutrino flux from 0.1 to 100 GeV, with each ingredient either trusted or validated, and with a quantified, data-grounded systematic in the sub-GeV region where it matters most. The geomagnetic layer is production-ready; the absolute normalisation is data-anchored to ~10 % above 1 GeV; the directional observables match the authoritative Honda 3D tables. The sub-GeV central value is now anchored to a dedicated low-energy dataset via the recommended **hybrid base** (`base_model="hybrid"` + GSF primary): the raw-MCEq sub-GeV deficit is traced to the H3a primary (~32 % below the AMS-02/BESS/PAMELA-fitted Global Spline Fit at 10 GeV, the sub-GeV parent region), and blending GSF-anchored MCEq below the physics-derived crossover E0=1.7 GeV (the 5 GeV muon-calibration floor mapped to E_ν≈E_μ/3; scan sensitivity ≤6% below 1 GeV) with muon-calibrated daemonflux above reproduces the Honda **solar-minimum** tables (the epoch of the reference, pinned empirically against Bartol's fmin/fmax) to ±11 % or better for both flavours, at every zenith, over 0.14–10 GeV. The natural next step is to integrate the back-traced cutoff as the default in the released package (it is already exposed through the API, with cached maps making it fast enough).
