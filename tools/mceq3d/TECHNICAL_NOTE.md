...
The directional observables, which are ratios and therefore independent of the absolute base, are reproduced well. The engine recovers the **up/down asymmetry** (up-going flux exceeds down-going at fixed energy, as in Honda), the **sec θ horizon enhancement**, which now tracks Honda to ≈3–6 % across 0.3–100 GeV (1.27 vs 1.23 at 1 GeV; earlier drafts showed a prototype curve without E_off that undershot below a few GeV). The **East–West amplitude** has the correct sign, vanishes above ~10 GeV as it must, and now **tracks Honda to a few percent** through the peak (2.53 vs 2.51 at 0.6 GeV, 2.10 vs 2.04 at 1.1 GeV, 1.87 vs 1.83 at 1.5 GeV), with an 8–14 % overshoot only at the band edges. This resolves the ≈20 % deficit of earlier drafts, whose cause we pinned down: (i) a **rigidity-scan cap** — the cutoff back-trace scanned only to 20 GV, but the near-horizon East cutoff at Kamioka exceeds that and was clamped, under-suppressing the East (the penumbra width was tested and ruled out: sharpening it saturated ~18 % below Honda); raising the ceiling to 40 GV restores the contrast but overshoots with a single sharp cutoff; and (ii) **production-cone averaging** — the parent primary arrives from a spread of directions (the production cone), each with its own cutoff, so averaging G over that cone (the geomagnetic analogue of E_off) smears the single-cutoff amplitude onto Honda. Both are implemented and default-on for E–W (`cone_cutoff=True`, on a cached full-sky IGRF cutoff map). The **νe/νμ flavour ratio** — the sharpest, most model-independent test, set by the π→μ→e chain — agrees with Honda to 0.7–4 % across the band.
