...
``N``. The single-step *variance* is the NA61-validated ``<theta^2>(E)``; the
shape is forward-Gaussian. (A real-kernel path, :func:`single_production_f1` +
numerical Legendre, is provided but is statistics-limited and not used here.)
...
def main(argv=None):
    ...
    print("  E[GeV]   P_N(bounded)   small-angle(unbounded)")
    for e, a, b in zip(energies, sn, sa):
        print(f"  {e:7.2f}     {a:7.2f}            {b:8.2f}")

    if args.plot:
        _plot(energies, sn, sa, args)


def _plot(energies, sn, sa, args):
    import matplotlib
    ...
    ax.loglog(energies, sn, "C0o-", lw=2, label=r"$P_N$ full-angle (bounded)")
