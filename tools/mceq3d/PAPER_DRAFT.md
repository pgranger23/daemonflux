...
$$
\Phi_s(E,\cos\theta_z,\varphi) \;=\; \Phi^{1D}_s\!\left(E,\,|\cos\theta_z|\right)\; R_s\!\left(E,\cos\theta_z\right)\; G_s\!\left(E,\,R_c(\cos\theta_z,\varphi)\right),
\tag{1}
$$

where $\Phi^{1D}_s$ is the per-zenith 1D cascade (carrying the curved-atmosphere sec θ, Section 2), $R_s=\Phi_{3D}/\Phi_{1D}$ is the production-angle angular redistribution (Section 3.6), and $G_s \in [0,1]$ is the cascade-correct response to removing primaries below the directional rigidity cutoff $R_c$. Each factor is a *validated* piece: $\Phi^{1D}$ is muon-calibrated, $R$ is driven by NA61 data, $G$ is the first-principles rigidity response. The construction is genuinely 3D — every mechanism (curved geometry, off-axis production, geomagnetic anisotropy) enters — while remaining a fast deterministic product rather than a Monte-Carlo. The absolute value $|\cos\theta_z|$ enters the base because atmospheric production is up/down symmetric; the up/down asymmetry of the *observed* flux arises through $R_c$ (Section 3.5). $R\to 1$ and $G\to 1$ at high energy, recovering the calibrated 1D flux. (Setting $R=1$ gives the fast factorised path; it differs from the full construction by only the $\lesssim 2\%$ of Section 3.6.)
...
and the cumulative angular spread over the production-and-decay chain is captured by $\sigma_\theta(E)$, measured from the regenerated kernels (variance-only Fokker–Planck and full multipole $P_N$ transport agree). This is folded into the delivered flux as the redistribution factor $R_s(E,\cos\theta_z)$ of Eq. (1): the per-zenith base is convolved on the sphere with $\sigma_\theta(E)$ (flux-conserving; `angular_factor`, `solve(full_3d=True)`), which moves flux between arrival directions exactly as off-axis production does in a 3D shower. Since the curved-atmosphere sec θ is already in the per-zenith base, only the genuine off-axis redistribution is added (no double counting; the crude geometric `spherical_geometry` estimate is *not* layered on for that reason). The effect is a $\sim 1$–$2\%$ zenith redistribution sub-GeV (a slight horizon deficit / vertical excess), vanishing above a few GeV — validated at the kinematic level against NA61 (Section 4.2) and, at the flux level, by the preserved absolute Honda/Bartol agreement when it is folded in (Section 4.1). Because this convolution is flux-conserving it does not create the *net* near-horizon enhancement seen in the full-3D references; that larger, distinct term is treated separately in Section 4.5.
...
The directional observables are ratios and are therefore independent of the absolute base. The engine recovers the up/down asymmetry (up-going exceeds down-going at fixed energy, as in Honda), the East–West amplitude at Kamioka ($2.4$ here versus $2.1$ in Honda at $1\,\mathrm{GeV}$, with the correct sub-GeV peak and high-energy vanishing), and the $\sec\theta$ horizon enhancement ($2.17$ versus $2.19$ at $100\,\mathrm{GeV}$) (Figure 6). The $\nu_e/\nu_\mu$ flavour ratio — the sharpest, most model-independent test, set by the $\pi\to\mu\to e$ chain — agrees with Honda to $0.7$–$4\%$ across the band. Folding in the full-3D production-angle redistribution ($R$, `full_3d=True`) shifts the vertical $\nu_\mu$ by only $\approx 1\%$ (e.g. Table 1 ratios move $1.30\to1.32$ at $0.5\,\mathrm{GeV}$, $1.11\to1.12$ at $1\,\mathrm{GeV}$, unchanged at $3$–$100\,\mathrm{GeV}$): the complete deterministic-3D flux reproduces the full-3D references just as the factorised one does, confirming the redistribution is a genuine but small term.
...
### 4.5 The near-horizon 3D excess

The one place the deterministic construction is genuinely incomplete is the **sub-GeV near-horizon zenith shape**. Comparing the delivered zenith shape $\Phi(\cos\theta_z)/\Phi(\mathrm{vertical})$ to the full-3D references exposes a real, *large* enhancement that the per-zenith cascade and the flux-conserving redistribution $R$ do not produce: at the horizon ($\cos\theta_z\!\approx\!0.05$) the references give horizon/vertical $\approx 1.8$ at $0.3\,\mathrm{GeV}$, $1.4$ at $0.5\,\mathrm{GeV}$, $1.25$ at $1\,\mathrm{GeV}$, converging to our value by $\approx 3\,\mathrm{GeV}$, whereas our uncorrected shape is $\approx 0.9$–$1.05$ there (Figure 11). This is the classic 3D horizontal enhancement from off-axis production in the curved atmosphere; it is *robust*, Honda and Bartol — two independent 3D Monte-Carlos — agreeing on it to $\approx 5\%$.

A first-principles deterministic derivation of this term is the full 3D-Monte-Carlo problem (the reason those codes exist). We therefore model it **explicitly but reference-anchored**: a multiplicative shape correction $H(E,\cos\theta_z)$, equal to the ratio of the Honda zenith shape to ours (unity at the vertical and above a few GeV; `build_horizon_excess.py`, `solve(horizon_excess=True)`). Because $H$ is *anchored to Honda*, agreement with Honda is no longer an independent test — but agreement with **Bartol is**, since Bartol is not used to build $H$. After folding $H$ in, the delivered zenith shape reproduces the independent Bartol calculation to within the $\approx 2$–$7\%$ Honda–Bartol spread across $0.3$–$3\,\mathrm{GeV}$ (e.g. at the $0.3\,\mathrm{GeV}$ horizon: uncorrected $0.94\to$ corrected $1.89$, Honda $1.89$, Bartol $1.76$). This turns the excess from a bounded-but-unmodelled residual into an explicit, cross-validated term. It is honestly *not* first-principles (it inherits the $\sim 5\%$ inter-reference uncertainty and is derived at Kamioka as an assumed-geometric, site-independent factor); a from-scratch off-axis-production cascade remains the open item.

![**Figure 11.** Near-horizon 3D excess. Delivered $\nu_\mu$ zenith shape at Kamioka before (dashed) and after (red) the reference-anchored correction $H$, against Honda and the independent Bartol calculation, at $0.3$ and $1\,\mathrm{GeV}$. The uncorrected deterministic flux misses the sub-GeV horizontal enhancement; with $H$ it reproduces the full-3D references to their mutual $\sim 5\%$.](horizon_excess.png)
...
We are explicit about the trust boundary. The delivered engine (Section 3, Eq. 1) is the complete deterministic-3D construction — curved per-zenith cascade $\times$ production-angle redistribution $\times$ geomagnetic cutoff $\times$ solar — validated absolutely against Honda and Bartol and at the kinematic level against NA61. Its one deterministic gap is the **net** off-axis production geometry: the redistribution $R$ is flux-conserving, so it captures the directional smearing of production but not the net **sub-GeV near-horizon excess**, which is large (horizon/vertical $\approx 1.8$ at $0.3\,\mathrm{GeV}$, Section 4.5) — *not* the $\sim 1$–$2\%$ we had earlier assumed. Rather than leave it as a bounded residual, we model it **explicitly** as the reference-anchored correction $H$ of Section 4.5, cross-validated against the independent Bartol calculation; this is honestly not first-principles (it carries the $\sim 5\%$ Honda–Bartol spread and is derived at Kamioka as an assumed-geometric factor). A from-scratch deterministic off-axis-production cascade — which would make $H$ first-principles — remains the principal open item; the exploratory `spherical_streaming`/`spherical_cascade`/`spherical_geometry` modules are steps toward it but are not yet absolutely validated.
