...
The daemonflux **nuisance-parameter covariance** is now propagated to the directional flux (Sections 2 and 5): the $1\sigma$ calibration band is delivered exactly, and the *full* correlated covariance follows by scaling each daemonflux parameter gradient by $G\,S$ — the remaining engineering step is simply to export that scaled gradient matrix for a global fit (the scaling itself is trivial and validated by the preserved fractional error). One capability remains deliberately out of scope: the **correlated muon** accompanying a down-going neutrino — the atmospheric self-veto passing fraction relevant to neutrino telescopes — is an event-level quantity that an inclusive flux model (this work, MCEq, or daemonflux) cannot provide; it requires a dedicated calculation such as nuVeto [25].
